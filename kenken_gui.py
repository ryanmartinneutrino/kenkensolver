...
class KenKenApp:
    ...
    def _build_ui(self) -> None:
        ...
        ttk.Button(ctrl, text="Delete Block", command=self.delete_block_action).grid(
            row=1, column=1, sticky="ew", padx=(6, 0), pady=(8, 0)
        )

        ttk.Label(ctrl, text="Operator:").grid(row=2, column=0, sticky="w", pady=(10, 2))
        ...
        ttk.Label(ctrl, text="Target:").grid(row=3, column=0, sticky="w", pady=2)
        ttk.Entry(ctrl, textvariable=self.target_var, width=10).grid(row=3, column=1, sticky="w", pady=2)

        ttk.Button(ctrl, text="Apply Rule", command=self.apply_rule_to_current).grid(
        ...
        hint = (
            "How to use:\n"
            "1) Click New Block.\n"
            "2) Click grid cells to add/remove from this block.\n"
            "3) Select operator and enter target.\n"
            "4) Click Apply Rule (rule cell auto = top-left).\n"
            "5) Repeat until all cells are assigned, then Solve."
        )
        ttk.Label(ctrl, text=hint, justify="left", wraplength=230).grid(
    ...
    def new_grid(self) -> None:
        try:
            ...
            return
        self.clear_all()
        self.status_var.set("New empty grid created. Click New Block to begin.")

    def new_block(self) -> None:
        self.delete_mode = False

        if self.current_block_id is not None:
            current = self.blocks.get(self.current_block_id)
            if current and not current.is_defined and current.cells:
                messagebox.showinfo(
                    "Finish current block",
                    "Apply Rule or Delete Block for the current block before creating a new one.",
                )
                return
    ...
    def delete_block_action(self) -> None:
        ...
        if self.current_block_id is not None:
            current = self.blocks.get(self.current_block_id)
            if current and not current.is_defined:
                self._delete_block(current.block_id)
                self.status_var.set("Deleted current in-progress block.")
                self.redraw()
                return

        self.current_block_id = None
        self.delete_mode = True
        self.status_var.set("Delete mode: click any cell in a defined block to delete that block.")
        self.redraw()
    ...
    def on_canvas_click(self, event: tk.Event) -> None:
        ...
        if self.delete_mode:
            block_id = self.cell_to_block.get(cell)
            if block_id is None:
                self.status_var.set("Delete mode: clicked empty cell; no block deleted.")
                return
            confirmed = messagebox.askyesno(
                "Confirm block deletion",
                f"Delete block #{block_id} and remove all its cells/rule?",
            )
            if confirmed:
                self._delete_block(block_id)
                self.status_var.set(f"Deleted block #{block_id}.")
            else:
                self.status_var.set("Block deletion canceled.")
            self.delete_mode = False
            self.redraw()
            return

        if self.current_block_id is None:
            self.status_var.set("Click New Block first (or Delete Block to remove an existing block).")
            return

        block = self.blocks[self.current_block_id]
        if block.is_defined:
            self.status_var.set("Current block already defined. Click New Block to create the next block.")
            return
        ...
        if owner is None:
            ...
            self.status_var.set(f"Cell {cell} already belongs to block #{owner}.")
            return

        self.redraw()

    def apply_rule_to_current(self) -> None:
        self.delete_mode = False
        block = self.blocks.get(self.current_block_id) if self.current_block_id is not None else None
        if not block:
            self.status_var.set("No active block. Click New Block first.")
            return
        if not block.cells:
            messagebox.showerror("Invalid block", "Current block has no cells.")
            return
        ...
        block.op = op
        block.target = target
        block.rule_cell = min(block.cells)
        block.is_defined = True
        self.current_block_id = None
        self.solution_grid = None
        self.status_var.set(
            f"Applied rule to block #{block.block_id}. Rule cell set to top-left cell {block.rule_cell}."
        )
        self.redraw()
    ...
    def validate_before_solve(self) -> None:
        ...
        if set(self.cell_to_block) != expected_cells:
            missing = sorted(expected_cells - set(self.cell_to_block))
            raise ValueError(f"All cells must belong to a block before solving. Missing cells: {missing}")
        ...
        for block in self.blocks.values():
            if not block.is_defined:
                raise ValueError(f"Block #{block.block_id} is not defined yet. Click Apply Rule.")
            if not block.cells:
            ...
            if block.rule_cell is None or block.rule_cell not in block.cells:
                raise ValueError(f"Block #{block.block_id} must have one rule cell in the block.")
    ...
    def redraw(self) -> None:
        ...
        for r in range(n):
            for c in range(n):
                ...
                if self.current_block_id is not None and bid == self.current_block_id:
                    fill = "#ffe082"
            ...
            for r in range(n):
                for c in range(n):
                    ...
                    self.canvas.create_text(
                        x,
                        y,
                        text=str(self.solution_grid[r][c]),
